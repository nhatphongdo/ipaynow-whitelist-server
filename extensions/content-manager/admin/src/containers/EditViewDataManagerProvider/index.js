    ...
    try {
      ...
      try {
        ...
        const users = await request(
          getRequestUrl("plugins::users-permissions.user"),
          {
            method: "GET",
            headers,
            qs: {
              pushNotificationTokens_null: false,
              _limit: -1,
              _start: 0
            }
          }
        ...
        for (let i = 0; i < users.length; i++) {
          if (!users[i].pushNotificationTokens) {
            continue;
          }

          cleanedData.user = users[i].id;

          if (!cleanedData.scheduleTime) {
            cleanedData.scheduleTime = new Date().toISOString();
          }
          if (!cleanedData.sent) {
            cleanedData.sent = false;
          }
          if (!cleanedData.sending) {
            cleanedData.sending = false;
          }
          if (!cleanedData.retries) {
            cleanedData.retries = 0;
          }
          if (!cleanedData.read) {
            cleanedData.read = false;
          }

          formData.set("data", JSON.stringify(cleanedData));
