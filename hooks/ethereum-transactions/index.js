...
module.exports = (strapi) => {
  const hook = {
    ...
    initialize: async () => {
      ...
      eth.watchEthTransfer(
        lastBlock,
        [centralWallet, escrowWallet, storeWallet],
        hook.processEthTransaction,
        hook.updateBlock
      );
      eth.watchTokenTransfers(
        lastBlock,
        [centralWallet, escrowWallet, storeWallet],
        hook.processTokenTransaction
      );
    },
    ...
    processEthTransaction: async (tx) => {
      ...
      if (tx.to.toLowerCase() === eth.TOKEN_CONTRACT_ADDRESS.toLowerCase()) {
        ...
        if (txReceipt && txReceipt.logs && txReceipt.logs.length > 0) {
          ...
          if (func === eth.TRANSFER_TOPIC) {
            ...
            amount = eth.fromWei(eth.hexToNumberString(txReceipt.logs[0].data) + '0', 'gwei'); // Gwei has 9 digits
          }
        }
      ...
      if (
        !result &&
        tx.blockNumber &&
        tx.blockNumber < hook.minFailedBlockNumber
      ) {
        ...
        result &&
        tx.blockNumber &&
        tx.blockNumber > hook.lastBlockNumber
      ) {
        // If success, keep the latest block
    ...
    processTokenTransaction: async (txEvent) => {
      // Get transaction receipt
      const txReceipt = await eth.getTransactionReceipt(
        txEvent.transactionHash
      );
      const result =
        await strapi.services.transaction.updateBlockchainTransaction(
          eth.hexToAddress(txEvent.returnValues.from),
          eth.hexToAddress(txEvent.returnValues.to),
          txEvent.transactionHash,
          eth.fromWei(txEvent.returnValues.value.toString() + '0', "gwei"), // Gwei has 9 digits
          false,
          txReceipt ? txReceipt.status : null,
          txReceipt
        );
      ...
      if (
        !result &&
        txEvent.blockNumber &&
        txEvent.blockNumber < hook.minFailedBlockNumber
      ) {
        // If failed then keep the minimum block number
        ...
        result &&
        txEvent.blockNumber &&
        txEvent.blockNumber > hook.lastBlockNumber
      ) {
        // If success, keep the latest block
